...
public class DefaultRowTag extends ConfigurableTag
{
    ...
    * <br><code>false</code> - the header of the nested value list is rendered as a part of the top value list 
    * <br><code>true</code> - the header is rendered for each nested valuelist
    */
   private boolean nestedHeader = true;

   /**
    * Initialization is called at the beginning of <code>doStart</code>.
    * Subclasses have to call either <code>super.init()</code> or <code>super.doStart()</code>.
    * @throws JspException
    */
   ...
   public int doStartTag() throws JspException
   {
      ...
      {
         LOGGER.debug("Starting procesing rows of the table '" + getRootTag().getTableInfo().getId() + "' with the ValueList '"
      ...
      if (currentRowNumber == 0 && getRootTag() != null)
      {
         if (isEnabledHeader())
         {
            ...
            if (parentColumnTag == null || isNestedHeader())
            {
               ...
               renderHeaderRow(sb, getColumns(), getRootTag().getTableInfo(), getRootTag().getValueList().getValueListInfo(), getRootTag()
                     .getTableInfo().getParameters());
         ...
         throws JspException
   {
      ...
      {
         renderHeaderCell(sb, (ColumnInfo) iter.next(), tableInfo, vlInfo, parameters);
      }
   ...
   public int doEndTag() throws JspException
   {
      if (LOGGER.isDebugEnabled())
      {
         LOGGER.debug("End of processing rows of the table '" + getRootTag().getTableInfo().getId() + "' with the ValueList '"
               + getRootTag().getTableInfo().getName() + "'.");
      }
   ...
   protected int processIteration() throws JspException
   {
      if (getRootTag() != null && getRootTag().getValueList() != null && getRootTag().getValueList().hasNext())
      {
         ...
         if (LOGGER.isDebugEnabled())
         {
            LOGGER.debug("In the row no.'" + currentRowNumber + "' was setted the JavaBean '" + bean + "' in the pageContext.");
         }
         ...
         if (beanInPageScope == null || beanInPageScope instanceof Spacer)
         {
            ...
            if (LOGGER.isDebugEnabled())
            {
               LOGGER.debug("In the row no.'" + currentRowNumber + "' was not found any ValueList's List's (JavaBean) items to render.");
            }
         }
         else
         {
   ...
   public void setFocusRowNumber(String focusRowNumber)
   {
      try
      {
         this.focusRowNumber = Integer.parseInt(focusRowNumber);
      }
    ...
    * @param nestedHeader 
    *        <code>false</code> - the header of the nested value list is rendered as a part of the top value list 
    *        <code>true</code> - the header is rendered for each nested valuelist
    */
