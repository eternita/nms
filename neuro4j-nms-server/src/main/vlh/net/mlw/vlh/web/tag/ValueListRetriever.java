...
//      pageContext.getRequest().setAttribute(rootTag.getTableInfo().getName(), list);
      ValueList list = (ValueList) pageContext.getRequest().getAttribute(rootTag.getTableInfo().getName());
    if (list == null)
    {
       final String message = "Cannot locate the ValueList in the WebApplicationContext ";
       LOGGER.error(message);
       throw new JspException(message);
    }
