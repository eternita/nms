...
public class MultiColsResolvedVLHAdapter extends AbstractValueListAdapter {


	private EntryResolver resolver;

	public MultiColsResolvedVLHAdapter(EntryResolver resolver) {
		setAdapterType(DO_SORT);
		this.resolver = resolver;
	}
	
	public ValueList getValueList(ColumnEntries[] ids, String language, ValueListInfo info, int rowPerPage) 
	{
		...
		if (ids.length > 0)
			displayTotalRows = rowPerPage*(ids.length - 1) + ids[ids.length-1].getObjectCount();
		info.setTotalDisplayedNumberOfEntries(displayTotalRows);
